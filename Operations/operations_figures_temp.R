...
#nbertrand@usbr.gov
#updated by Chase Ehlo for WY25
#cehlo@usbr.gov 
#updated by Lillian McCormick for WY26
#lmccormick AT usbr DOT gov
...
#################################
#Read in OMRI from SacPAS -use if no operator data yet
#################################
...
#plots OMR data
omr1_plot <- data_import %>%
  #filter(measure== "OMR") %>% 
  ggplot(aes(x = date)) +
  geom_line(aes(y = omr_1)) +
  geom_hline(yintercept = -5000,  linewidth = 1, linetype = "dashed", color = "gray70") +
...
export_plot <- data_import %>%
  ...
  mutate(Facility = factor(Facility, levels = c('CCF', 'JPP'),
                           labels = c('Clifton Court Inflow (cfs)',
                                      'Jones Pumping Plant (cfs)'))) %>%
  ggplot(aes(x = date)) +
  labs(y = 'Exports (cfs)') +
  geom_col(aes(y = exports, fill = Facility)) +
...
final_figure <- omr1_plot/export_plot + plot_layout(height = c(1.5,1))
final_figure

ggsave(final_figure, file = 'Operations/outputs/omr_exports_2026.png', height = 5, width = 8)
